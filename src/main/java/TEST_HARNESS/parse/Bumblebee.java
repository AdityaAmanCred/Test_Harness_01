...
public class Bumblebee extends Parser {
    ...
    public boolean fetchResponse() {
        ...
        Request request = new Request.Builder().url(String.format(fetchProperty("BUMBLEBEE_FQDN"), this.envName)).method("POST", body)
        ...
        try {
            ...
            if (response.code() >= 200 && response.code() < 300) {
                ...
                log.warn("On " + this.envName + " ResponseCode: " + response.code() + " for " + this.fileName.split("\\.")[0]);
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
