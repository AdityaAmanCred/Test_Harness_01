...
import static TEST_HARNESS.utils.Util.createJSONMap;
import static TEST_HARNESS.utils.Util.fetchProperty;
import static TEST_HARNESS.utils.Util.getCommonFileNames;
import static TEST_HARNESS.utils.Util.getIssuerDetails;
import static TEST_HARNESS.utils.Util.readJsonFile;
import static TEST_HARNESS.utils.Util.removeRedundantDifference;
...
import TEST_HARNESS.result.pojos.SSPOJO;
...
public class Comparator {
    ...
    private void updateNullCheckMap(String fileName, ParserType parserType) {
        Map<String, Object> rightFlatMap = Util.flatten(filteredRightMap);
        Map<String, Object> leftFlatMap = (Config.getCompareAgainst() != CompareAgainst.STANDALONE) ? Util.flatten(filteredLeftMap) : null;
        ...
        for (String k : allfields) {
            if (rightFlatMap.containsKey(k) == true && (rightFlatMap.get(k) == null || rightFlatMap.get(k).toString().equals(""))) {
                ...
                Standalone obj = (Config.getCompareAgainst() == CompareAgainst.SECONDARY) ? new Variance(fileName, "null",
