...
public class ParsePdf {
    ...
    public void executeParsing(int numberOfPrimaryParsingThreads, int numberOfSecondaryParsingThreads, int primaryParserFetchRate,
            int secondaryParserFetchRate) {
        if (Application.getCompareAgainst() == CompareAgainst.SECONDARY && isValidParserSelection() == false) {
            System.exit(0);
        }
        ParsingExecutor parsingExecutor = new ParsingExecutor(numberOfPrimaryParsingThreads, numberOfSecondaryParsingThreads, primaryParserFetchRate,
                secondaryParserFetchRate);
        parsingExecutor.fetchAllResponses();
    }
